...
def setup_config():
    ...
    default_config = {
        ...
        "template": {
            "path": os.path.join(
                os.path.dirname(__file__), "templates/note_template.md"
            ),
            "variables": {},
        },
...
def process_single_file(input_path, output_dir, config):
    ...
    try:
        ...
        with ThreadPoolExecutor() as executor:
            future_to_page = {
                executor.submit(ocr_image_with_gemini, image, model_name, prompt): i
                for i, image in enumerate(images)
            }

            with tqdm(
                total=len(images), desc=f"Processing {os.path.basename(input_path)}"
            ) as pbar:
...
def get_input_paths(input_path):
    """Get list of input file paths from a file or directory."""
    input_paths = []
...
def process_command(args):
    ...
    input_paths = get_input_paths(args.input_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="A tool for processing handwritten notes."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    # Process command
    process_parser = subparsers.add_parser(
        "process", help="Process a PDF or image file."
    )
    process_parser.add_argument(
        "input_path",
        help="The path to the PDF/image file or directory containing files to process.",
    )
    process_parser.add_argument(
        "output_dir", help="The directory to save the output Markdown file."
    )
    ...
    config_parser = subparsers.add_parser(
        "config", help="Manage the configuration file."
    )
    config_parser.add_argument(
        "--setup", action="store_true", help="Create the default configuration file."
    )
